...
mixin(Request.prototype, {
	...
	_responseHandler: function(response) {
		...
		if (this._isRedirect(response) && this.options.followRedirects == true) {
			...
			var body = new Buffer(0), body2;

			// TODO Handle different encodings
			//response.setEncoding('utf8');

			response.on('data', function(chunk) {
				body2 = new Buffer(body.length + chunk.length) ;
				body.copy(body2, 0);
				chunk.copy(body2, body.length);
				body = body2;
			});

			response.on('end', function() {
				var m = response.headers['content-type'].match(/charset=(.+)$/i)

				if (m && !m[1].match(/utf-8$/i)) {
					try {
						var my_iconv = new Iconv(m[1], 'UTF-8//TRANSLIT//IGNORE');
						var my_buffer = my_iconv.convert(body);
						body = my_buffer.toString('utf8');
					} catch (e) {
	...
	_fireEvents: function(error, body, response) {
		if (parseInt(response.statusCode) >= 400) {
			this._respond('error', error, body, response);
		} else {
			this._respond('success', error, body, response);
		}
